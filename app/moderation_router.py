...
def create_moderation_router(session_factory: async_sessionmaker[AsyncSession]) -> Router:
    ...
    async def get_set(session: AsyncSession, chat_id: int, set_id: int) -> FilterSet | None:
        return (await session.execute(
            select(FilterSet).where(FilterSet.id == set_id, FilterSet.chat_id == chat_id)
        )).scalar_one_or_none()
    ...
    async def filterset_add(message: Message, bot: Bot) -> None:
        ...
        async with session_factory() as session:
            ...
            item = FilterSet(
                ...
                name=name,
                kind=kind,
                match_type="whole" if kind == "word" else "contains",
                action="delete",
                delete_message=True,
                exclude_admins=True,
    ...
    async def filteritem_add(message: Message, bot: Bot) -> None:
        ...
        async with session_factory() as session:
            ...
            exists = (await session.execute(
                select(FilterItem).where(FilterItem.filter_set_id == set_id, FilterItem.value == value)
            )).scalar_one_or_none()
            if exists:
                await message.answer("Такой элемент уже есть в наборе.")
                return
    ...
    async def filterset_action(message: Message, bot: Bot) -> None:
        ...
        if len(parts) < 3:
            await message.answer("Формат: /filterset_action <ID> <delete|warning|mute|ban> [секунды_для_mute]")
            return
        try:
            set_id = int(parts[1])
        except ValueError:
        ...
        if action not in {"delete", "warning", "mute", "ban"}:
            await message.answer("Действие: delete, warning, mute или ban.")
            return
        mute_seconds = None
        if action == "mute":
            if len(parts) != 4:
                await message.answer("Для mute укажи срок в секундах: /filterset_action <ID> mute <секунды>")
                return
            try:
                mute_seconds = int(parts[3])
            except ValueError:
                mute_seconds = 0
            if mute_seconds <= 0:
                await message.answer("Срок мута должен быть больше нуля.")
                return
        async with session_factory() as session:
            ...
            if filter_set is None:
                ...
                return
            filter_set.action = action
            filter_set.mute_seconds = mute_seconds
            await session.commit()
        extra = f", {mute_seconds} сек." if action == "mute" else ""
        await message.answer(f"✅ Набор #{set_id}: действие {action}{extra}")

    @router.message(Command("filterset_delete"), F.chat.type.in_({"group", "supergroup"}))
    async def filterset_delete(message: Message, bot: Bot) -> None:
        if not await require_admin(message, bot):
            return
        parts = (message.text or "").split()
        if len(parts) != 3 or parts[2].lower() not in {"on", "off"}:
            await message.answer("Формат: /filterset_delete <ID> <on|off>")
            return
        try:
            set_id = int(parts[1])
        except ValueError:
        ...
        async with session_factory() as session:
            ...
            if filter_set is None:
                await message.answer("Набор не найден.")
                return
            filter_set.delete_message = parts[2].lower() == "on"
            await session.commit()
    ...
    async def filterset_priority(message: Message, bot: Bot) -> None:
        ...
        parts = (message.text or "").split()
        if len(parts) != 3:
            await message.answer("Формат: /filterset_priority <ID> <число>")
            return
        try:
            set_id, priority = int(parts[1]), int(parts[2])
        except ValueError:
    ...
    async def filterset_reason(message: Message, bot: Bot) -> None:
        ...
        parts = (message.text or "").split(maxsplit=2)
        if len(parts) != 3:
            await message.answer("Формат: /filterset_reason <ID> <причина>")
            return
        try:
            set_id = int(parts[1])
        except ValueError:
    ...
    async def whitelist_add(message: Message, bot: Bot) -> None:
        ...
        if target is None or target.is_bot:
            await message.answer("Ответь /whitelist_add на сообщение пользователя.")
            return
        async with session_factory() as session:
            exists = (await session.execute(select(ModerationWhitelist).where(
                ModerationWhitelist.chat_id == message.chat.id,
                ModerationWhitelist.user_id == target.id,
            ))).scalar_one_or_none()
            if not exists:
                session.add(ModerationWhitelist(chat_id=message.chat.id, user_id=target.id))
                await session.commit()
    ...
    async def whitelist_remove(message: Message, bot: Bot) -> None:
        ...
        target = message.reply_to_message.from_user if message.reply_to_message else None
        if target is None:
            await message.answer("Ответь /whitelist_remove на сообщение пользователя.")
            return
        async with session_factory() as session:
            row = (await session.execute(select(ModerationWhitelist).where(
                ModerationWhitelist.chat_id == message.chat.id,
                ModerationWhitelist.user_id == target.id,
            ))).scalar_one_or_none()
            if row:
    ...
    async def warnings(message: Message) -> None:
        ...
        if target is None:
            return
        async with session_factory() as session:
            count = (await session.execute(select(func.count(ModerationWarning.id)).where(
                ModerationWarning.chat_id == message.chat.id,
                ModerationWarning.user_id == target.id,
            ))).scalar_one()
    ...
    async def filtersets(message: Message) -> None:
        ...
        for row in rows:
            ...
            lines.append(f"#{row.id} {'✅' if row.is_active else '❌'} {row.name} [{row.kind}] → {row.action}{mute}; delete={'on' if row.delete_message else 'off'}; p={row.priority}")
    ...
    async def modlog(message: Message, bot: Bot) -> None:
        ...
        for row in rows:
            lines.append(f"#{row.id} user={row.user_id} set={row.filter_set_id} match={row.matched_value!r} final={row.action} ok={row.telegram_ok}")
        await message.answer("\n".join(lines))
