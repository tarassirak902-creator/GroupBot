...
class AntiLinksMiddleware(BaseMiddleware):
    ...
    async def __call__(
        ...
        async with self.session_factory() as session:
            ...
            whitelist = {
                str(x).casefold().removeprefix("www.").strip(".")
                for x in (raw.get("link_whitelist") or [])
                if str(x).strip()
            }
        ...
        try:
            ...
            logger.info(
                "Anti-links could not delete chat_id=%s message_id=%s",
                event.chat.id,
                event.message_id,
            )
        if deleted:
        ...
        try:
            ...
            logger.exception(
                "Anti-links action failed for chat_id=%s user_id=%s",
                event.chat.id,
                event.from_user.id,
            )
        return await handler(event, data)
