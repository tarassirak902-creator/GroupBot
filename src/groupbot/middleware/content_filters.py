...
class ContentFiltersMiddleware(BaseMiddleware):
    ...
    async def __call__(
        ...
        if action is None or reason is None or source is None:
            return await handler(event, data)
        if not claim_automatic_moderation(data, source):
            return await handler(event, data)
        ...
        try:
            ...
            logger.info(
                "Content filter could not delete chat_id=%s message_id=%s",
                event.chat.id,
                event.message_id,
            )
        if deleted:
        ...
        try:
            ...
            logger.exception(
                "Content filter action failed for chat_id=%s user_id=%s",
                event.chat.id,
                event.from_user.id,
            )
