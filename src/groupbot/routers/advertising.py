...
def _sell_groups_keyboard(groups: list[tuple[int, str | None]]) -> InlineKeyboardMarkup:
    ...
    for chat_id, title in groups:
        rows.append([InlineKeyboardButton(text=f"🏠 {title or 'Группа'}"[:64], callback_data=f"ads:sell:group:{chat_id}")])
...
def _listing_text(listing: AdvertisingListing) -> str:
    ...
    lines = ["📢 <b>Рекламное объявление</b>", "", f"🏠 Группа: <b>{listing.group_title_snapshot}</b>", f"👥 Участников: <b>{members}</b>", ""]
    if listing.offers_post:
        ...
        lines.extend(["📣 <b>Рекламные посты</b>", f"⭐ Цена за сутки: <b>{listing.post_price_stars} ⭐</b>", f"⏱ Интервал публикации: <b>{hours_text} ч.</b>", ""])
    if listing.offers_mandatory:
        terms = listing.mandatory_terms_json or {}
        mode = terms.get("mode")
        unit = "за 1 день" if mode == "days" else "за 1 подписчика"
        lines.extend(["✅ <b>Обязательная подписка</b>", f"📐 Расчёт: <b>{'по дням' if mode == 'days' else 'по количеству подписчиков'}</b>", f"⭐ Цена {unit}: <b>{listing.mandatory_price_stars} ⭐</b>"])
    return "\n".join(lines)
...
def create_advertising_router(session_factory: async_sessionmaker[AsyncSession], settings: Settings) -> Router:
    ...
    async def _save_listing(state: FSMContext, user_id: int) -> AdvertisingListing | None:
        ...
        async with session_factory() as session:
            async with session.begin():
                ownership = (await session.execute(select(Group, GroupOwner).join(GroupOwner, GroupOwner.chat_id == Group.chat_id).where(Group.chat_id == chat_id, GroupOwner.user_id == user_id, GroupOwner.is_current.is_(True), Group.status == GroupStatus.active.value))).first()
                if ownership is None:
                    return None
                group, _owner = ownership
                listing = (await session.execute(select(AdvertisingListing).where(AdvertisingListing.chat_id == chat_id).with_for_update())).scalar_one_or_none()
                if listing is None:
                    listing = AdvertisingListing(owner_user_id=user_id, chat_id=chat_id, group_title_snapshot=group.title or "Группа", member_count_snapshot=int(data.get("member_count") or 0), offers_post=bool(data.get("offers_post")), offers_mandatory=bool(data.get("offers_mandatory")))
                    session.add(listing)
                else:
                ...
                if listing.offers_mandatory:
                    ...
                    listing.mandatory_terms_json = {"mode": mode, "price_unit": "day" if mode == "days" else "subscriber"}
                else:
                    listing.mandatory_price_stars = None
                    listing.mandatory_terms_json = None
                await session.flush()
                listing_id = listing.id
            return (await session.execute(select(AdvertisingListing).where(AdvertisingListing.id == listing_id))).scalar_one()
    ...
    async def _edit_prompt(message: Message, state: FSMContext, text: str, markup: InlineKeyboardMarkup | None = None) -> None:
        ...
        if isinstance(prompt_message_id, int):
            try:
                await message.bot.edit_message_text(chat_id=message.chat.id, message_id=prompt_message_id, text=text, parse_mode="HTML", reply_markup=markup)
                return
            except Exception:
                pass
    ...
    async def advertising_home_message(message: Message, state: FSMContext) -> None:
        await state.clear()
        await message.answer("🟣 <b>Mimorus · Реклама</b>\n\nПокупайте рекламные размещения или выставляйте свою подключённую группу как площадку.", parse_mode="HTML", reply_markup=_advertising_keyboard())

    @router.callback_query(F.data == "ads:home")
    async def advertising_home(callback: CallbackQuery, state: FSMContext) -> None:
        await state.clear()
        if callback.message is not None:
            await callback.message.edit_text("🟣 <b>Mimorus · Реклама</b>\n\nПокупайте рекламные размещения или выставляйте свою подключённую группу как площадку.", parse_mode="HTML", reply_markup=_advertising_keyboard())
    ...
    async def sell_advertising(callback: CallbackQuery, state: FSMContext) -> None:
        ...
        if callback.message is None:
            return
        if not groups:
            await callback.message.edit_text("💼 <b>Продать рекламу</b>\n\nДля создания объявления нужна хотя бы одна активная подключённая группа, которой вы владеете.", parse_mode="HTML", reply_markup=_advertising_back_keyboard())
            await callback.answer()
            return
        await callback.message.edit_text("💼 <b>Продать рекламу</b>\n\nВыберите группу, для которой хотите создать рекламное объявление:", parse_mode="HTML", reply_markup=_sell_groups_keyboard(groups))
        await callback.answer()
    ...
    async def choose_sell_group(callback: CallbackQuery, bot: Bot, state: FSMContext) -> None:
        ...
        if chat_id not in groups:
            await callback.answer("Эта группа вам не принадлежит или не активна.", show_alert=True); return
        try:
            member_count = await bot.get_chat_member_count(chat_id)
        except Exception:
            member_count = 0
        await state.clear(); await state.update_data(chat_id=chat_id, group_title=groups[chat_id] or "Группа", member_count=member_count, prompt_message_id=callback.message.message_id)
        await callback.message.edit_text("💼 <b>Продать рекламу</b>\n\n" + f"🏠 Группа: <b>{groups[chat_id] or 'Группа'}</b>\n" + f"👥 Участников: <b>{member_count:,}</b>\n\n".replace(",", " ") + "Что вы хотите продавать?", parse_mode="HTML", reply_markup=_offer_type_keyboard(chat_id)); await callback.answer()
    ...
    async def choose_offer_type(callback: CallbackQuery, state: FSMContext) -> None:
        ...
        if len(parts) != 5:
            await callback.answer("Некорректный формат.", show_alert=True); return
        try: chat_id = int(parts[3])
        except ValueError:
            await callback.answer("Некорректная группа.", show_alert=True); return
        kind = parts[4]; data = await state.get_data()
        if data.get("chat_id") != chat_id or kind not in {"post", "mandatory", "both"}:
            await callback.answer("Мастер создания объявления устарел. Начните заново.", show_alert=True); return
        offers_post = kind in {"post", "both"}; offers_mandatory = kind in {"mandatory", "both"}
        ...
        if offers_post:
            ...
            await callback.message.edit_text("📣 <b>Рекламные посты</b>\n\nВведите стоимость размещения рекламного поста <b>за сутки</b> в Telegram Stars.\n\nОтправьте только целое положительное число, например: <code>50</code>.", parse_mode="HTML")
        else:
            await callback.message.edit_text("✅ <b>Обязательная подписка</b>\n\nКак вы хотите рассчитывать ОП?", parse_mode="HTML", reply_markup=_mandatory_mode_keyboard())
        await callback.answer()
    ...
    async def post_price(message: Message, state: FSMContext) -> None:
        ...
        if value <= 0:
            await _edit_prompt(message, state, "📣 <b>Рекламные посты</b>\n\nЦена должна быть целым положительным числом Stars. Попробуйте ещё раз."); return
        await state.update_data(post_price_stars=value); await state.set_state(AdvertisingListingState.waiting_post_interval)
    ...
    async def post_interval(message: Message, state: FSMContext) -> None:
        ...
        if value <= 0:
            await _edit_prompt(message, state, "📣 <b>Рекламные посты</b>\n\nИнтервал должен быть целым положительным количеством часов. Попробуйте ещё раз."); return
        await state.update_data(post_interval_hours=value); data = await state.get_data()
        if data.get("offers_mandatory"):
            await state.set_state(None); await _edit_prompt(message, state, "✅ <b>Обязательная подписка</b>\n\nКак вы хотите рассчитывать ОП?", _mandatory_mode_keyboard()); return
        listing = await _save_listing(state, message.from_user.id)
        if listing is None:
            await state.clear(); await message.answer("Не удалось сохранить объявление. Проверьте, что группа активна и принадлежит вам."); return
        prompt_message_id = (await state.get_data()).get("prompt_message_id"); await state.clear()
        if isinstance(prompt_message_id, int):
            try:
                await message.bot.edit_message_text(chat_id=message.chat.id, message_id=prompt_message_id, text="✅ <b>Рекламное объявление опубликовано</b>\n\n" + _listing_text(listing), parse_mode="HTML", reply_markup=_listing_created_keyboard(listing.id)); return
            except Exception: pass
        await message.answer("✅ <b>Рекламное объявление опубликовано</b>\n\n" + _listing_text(listing), parse_mode="HTML", reply_markup=_listing_created_keyboard(listing.id))
    ...
    async def mandatory_mode(callback: CallbackQuery, state: FSMContext) -> None:
        ...
        if mode not in {"days", "subscribers"}:
            await callback.answer("Некорректный способ расчёта.", show_alert=True); return
        data = await state.get_data()
        if not data.get("offers_mandatory"):
            await callback.answer("Мастер создания объявления устарел. Начните заново.", show_alert=True); return
        await state.update_data(mandatory_mode=mode); await state.set_state(AdvertisingListingState.waiting_mandatory_price)
        unit = "за 1 день" if mode == "days" else "за 1 подписчика"
        await callback.message.edit_text("✅ <b>Обязательная подписка</b>\n\n" + f"Введите стоимость <b>{unit}</b> в Telegram Stars.\n\n" + "Отправьте только целое положительное число.", parse_mode="HTML"); await callback.answer()
    ...
    async def mandatory_price(message: Message, state: FSMContext) -> None:
        ...
        await state.update_data(mandatory_price_stars=value); listing = await _save_listing(state, message.from_user.id)
        if listing is None:
            await state.clear(); await message.answer("Не удалось сохранить объявление. Проверьте, что группа активна и принадлежит вам."); return
        prompt_message_id = (await state.get_data()).get("prompt_message_id"); await state.clear()
        if isinstance(prompt_message_id, int):
    ...
    async def buy_advertising(callback: CallbackQuery) -> None:
        if callback.message is None: return
        async with session_factory() as session:
            rows = (await session.execute(select(AdvertisingListing).where(AdvertisingListing.is_active.is_(True), AdvertisingListing.owner_user_id != callback.from_user.id).order_by(AdvertisingListing.updated_at.desc(), AdvertisingListing.id.desc()).limit(50))).scalars().all()
        if not rows:
    ...
    async def my_sales(callback: CallbackQuery) -> None:
        ...
        await callback.message.edit_text(text, parse_mode="HTML", reply_markup=_listings_keyboard(list(rows), own=True)); await callback.answer()

    @router.callback_query(F.data.startswith("ads:listing:"))
    async def open_listing(callback: CallbackQuery) -> None:
        if callback.message is None: return
        try: listing_id = int((callback.data or "").rsplit(":", 1)[1])
        except (ValueError, IndexError):
        ...
        own = listing.owner_user_id == callback.from_user.id; keyboard_rows: list[list[InlineKeyboardButton]] = []
        if own:
            keyboard_rows.append([InlineKeyboardButton(text="✏️ Изменить", callback_data=f"ads:sell:group:{listing.chat_id}")]); keyboard_rows.append([InlineKeyboardButton(text="◀️ Мои продажи", callback_data="ads:my_sales")])
        else:
            keyboard_rows.append([InlineKeyboardButton(text="📨 Отправить запрос", callback_data=f"ads:request:{listing.id}")]); keyboard_rows.append([InlineKeyboardButton(text="◀️ Купить рекламу", callback_data="ads:buy")])
        keyboard_rows.append([InlineKeyboardButton(text="◀️ Реклама", callback_data="ads:home")])
        await callback.message.edit_text(_listing_text(listing), parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=keyboard_rows)); await callback.answer()
    ...
    async def my_buys(callback: CallbackQuery) -> None:
        if callback.message is not None:
            await callback.message.edit_text("📋 <b>Мои покупки</b>\n\nЗаявки и активные рекламные сделки появятся здесь после отправки первого запроса.", parse_mode="HTML", reply_markup=_advertising_back_keyboard())
        await callback.answer()

    @router.callback_query(F.data == "ads:reviews")
    async def advertising_reviews(callback: CallbackQuery) -> None:
        if callback.message is not None:
            await callback.message.edit_text("⭐ <b>Отзывы и споры</b>\n\nОтзывы будут доступны после завершённых рекламных сделок. Спор можно будет открыть во время активной рекламы и до окончательного закрытия сделки.", parse_mode="HTML", reply_markup=_advertising_back_keyboard())
        await callback.answer()
    ...
    async def creator_advertising_home(callback: CallbackQuery) -> None:
        ...
        if callback.message is not None:
            await callback.message.edit_text("📢 <b>Creator-реклама</b>\n\nУправление рекламной частью Mimorus:", parse_mode="HTML", reply_markup=_creator_advertising_keyboard())
        await callback.answer()
    ...
    async def creator_advertising_section(callback: CallbackQuery) -> None:
        ...
        labels = {"marketplace": "📣 Рекламная площадка", "mandatory": "🔔 Обязательные подписки", "reviews": "⭐ Отзывы и споры"}
        label = labels.get(section)
        if label is None: return
        if callback.message is not None:
            await callback.message.edit_text(f"{label}\n\nCreator-инструменты этого блока будут подключены к общей рекламной модели без отдельного параллельного маркетплейса.", reply_markup=_creator_advertising_back_keyboard())
        await callback.answer()
