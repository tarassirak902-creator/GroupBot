...
def _member_status(member) -> str:
    ...
    if status == "kicked": return MemberStatus.banned.value
    if status == "left": return MemberStatus.left.value
    if status == "restricted" and not bool(getattr(member, "is_member", True)): return MemberStatus.left.value
...
async def _store_member_status(session: AsyncSession, *, chat_id: int, user, status: str, rejoined: bool) -> None:
    ...
    if status == MemberStatus.member.value:
        ...
        if rejoined: update_values.update({"joined_at": func.now(), "last_activity_at": func.now()})
    else:
        values["left_at"] = func.now(); update_values["left_at"] = func.now()
...
async def _apply_credit(op: AdvertisingManualOp, credit: AdvertisingManualOpCredit, *, reason: str) -> None:
...
async def _credit_manual_op_join(session: AsyncSession, *, chat_id: int, user_id: int, invite_url: str | None) -> None:
    ...
    ops = list((await session.execute(select(AdvertisingManualOp).where(
        AdvertisingManualOp.target_chat_id == chat_id, AdvertisingManualOp.target_url == invite_url,
    ...
    for op in ops:
        ...
        if credit is None:
            credit = AdvertisingManualOpCredit(op_id=op.id, user_id=user_id, satisfied=False, counted=False, reason="pending_op")
            session.add(credit)
        await _apply_credit(op, credit, reason="invite_link_join")
        logger.info("MANUAL_OP_CREDIT_ADD op_id=%s chat_id=%s user_id=%s progress=%s quantity=%s via=invite", op.id, chat_id, user_id, op.progress_count, op.quantity)


async def _credit_latest_pending_manual_op(session: AsyncSession, *, chat_id: int, user_id: int) -> None:
    """Fallback for Telegram chat_member updates that omit invite_link.

    The middleware records a pending_op only after this user is blocked in source A
    and shown that campaign's subscription button. On a confirmed left->member event
    in target B, credit only the most recently shown pending campaign for that target.
    """
    row = (await session.execute(
        ...
        .where(
            ...
            AdvertisingManualOpCredit.reason == "pending_op",
            AdvertisingManualOpCredit.satisfied.is_(False),
            AdvertisingManualOpCredit.counted.is_(False),
            or_(AdvertisingManualOp.mode == "unlimited", AdvertisingManualOp.mode == "days", and_(AdvertisingManualOp.mode == "subscribers", AdvertisingManualOp.progress_count < AdvertisingManualOp.quantity)),
        )
        .order_by(AdvertisingManualOpCredit.credited_at.desc(), AdvertisingManualOpCredit.id.desc())
        .limit(1)
        .with_for_update()
    )).first()
    if row is None:
        logger.info("MANUAL_OP_PENDING_SKIP chat_id=%s user_id=%s reason=no_pending_campaign", chat_id, user_id)
        return
    credit, op = row
    await _apply_credit(op, credit, reason="pending_join_confirmed")
    logger.info("MANUAL_OP_PENDING_CREDIT op_id=%s source_chat=%s target_chat=%s user_id=%s progress=%s quantity=%s", op.id, op.source_chat_id, chat_id, user_id, op.progress_count, op.quantity)


async def _uncount_manual_op_leave(session: AsyncSession, *, chat_id: int, user_id: int) -> None:
    rows = list((await session.execute(
        select(AdvertisingManualOpCredit, AdvertisingManualOp).join(AdvertisingManualOp, AdvertisingManualOp.id == AdvertisingManualOpCredit.op_id)
        .where(AdvertisingManualOp.target_chat_id == chat_id, AdvertisingManualOp.status == "active", AdvertisingManualOpCredit.user_id == user_id, AdvertisingManualOpCredit.counted.is_(True)).with_for_update()
    )).all())
...
async def _drop_stale_assignment(session: AsyncSession, *, chat_id: int, user_id: int, status: str) -> None:
    ...
    role = None; was_reserve = False
    if assignment is not None:
        was_reserve = bool(assignment.is_reserve)
        if assignment.role_id is not None: role = (await session.execute(select(AdminRole).where(AdminRole.id == assignment.role_id))).scalar_one_or_none()
        if role is not None and role.name != HELPER_ROLE: await detach_helpers_from_mentor(session, chat_id=chat_id, mentor_id=user_id, actor_id=None, reason=f"mentor_{status}")
    ...
    if promotion is not None: await session.delete(promotion)
...
async def _release_telegram_admin_state_after_manual_demotion(session: AsyncSession, *, chat_id: int, user_id: int, actor_id: int | None) -> tuple[bool, bool]:
    ...
    if assignment is not None and assignment.is_reserve:
        ...
        if assignment.role_id is None: await session.delete(assignment)
        else: assignment.is_reserve = False
...
def create_member_status_sync_router(session_factory: async_sessionmaker[AsyncSession]) -> Router:
    ...
    async def member_status_changed(event: ChatMemberUpdated) -> None:
        ...
        old_status = _member_status(event.old_chat_member); new_status = _member_status(event.new_chat_member)
        raw_old = _status_value(event.old_chat_member); raw_new = _status_value(event.new_chat_member)
        ...
        if old_status == new_status and raw_old == raw_new: return
        async with session_factory() as session:
            async with session.begin():
                ...
                if known_group is None:
                    logger.info("MANUAL_OP_MEMBER_SKIP chat_id=%s user_id=%s reason=unknown_group", event.chat.id, user.id); return
                await upsert_user(session, user)
                actor_id = None
                if event.from_user is not None and not event.from_user.is_bot:
                    await upsert_user(session, event.from_user); actor_id = event.from_user.id
                await _store_member_status(session, chat_id=event.chat.id, user=user, status=new_status, rejoined=rejoined)
                if rejoined:
                    if invite_url: await _credit_manual_op_join(session, chat_id=event.chat.id, user_id=user.id, invite_url=invite_url)
                    else: await _credit_latest_pending_manual_op(session, chat_id=event.chat.id, user_id=user.id)
                if left: await _uncount_manual_op_leave(session, chat_id=event.chat.id, user_id=user.id)
                removed_special_statuses: list[str] = []; reserve_cleared = False; promotion_tracking_released = False
                if new_status in {MemberStatus.left.value, MemberStatus.banned.value}:
                    ...
                    if removed_special_statuses: await write_audit(session, "group.special_statuses_removed_on_member_exit", chat_id=event.chat.id, actor_user_id=None, target_type="user", target_id=str(user.id), payload={"member_status": new_status, "statuses": removed_special_statuses})
