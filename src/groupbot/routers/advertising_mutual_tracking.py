...
async def _track_manual_leave(
    ...
    *,
    ...
    """Remove a live subscriber credit only after a voluntary leave.

    Telegram reports a normal voluntary leave as ``left``. Administrative removal
    or ban is reported as ``kicked``/``banned`` and must keep the already earned
    campaign progress while permanently satisfying OP for that user.
    """
    ...
    for credit, op in credits:
        if new_status in {"kicked", "banned"}:
            # An admin-side restriction is not the user's fault. Keep any +1 that
            # was already earned, and never require this OP from the user again.
        ...
        # A pending join request is already a valid advertising result. A later
        # Telegram member update must not undo that result merely because the user
        # is not yet a member. Only a real joined subscriber credit is reversible.
        if credit.reason != "joined":
            continue
...
def create_advertising_mutual_tracking_router(session_factory: async_sessionmaker[AsyncSession]) -> Router:
    ...
    async def manual_private_entry(callback: CallbackQuery) -> None:
        ...
        buttons = [[InlineKeyboardButton(
            text=(title or "Группа")[:60],
            callback_data=f"ads:manual:source:{chat_id}",
        )] for chat_id, title in rows]
        if callback.message is not None:
            await callback.message.edit_text(
                "Выберите группу, в которой нужно включить ОП:",
                ...
                if new_is_member:
                    ...
                    previous = list((await session.execute(
                        ...
                        .join(
                            ...
                            AdvertisingMutualOpDirection.id == AdvertisingMutualOpMember.direction_id,
                        )
                        .where(
