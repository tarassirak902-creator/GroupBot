...
def _is_member(status: str, member) -> bool:
    return status in {"member", "administrator", "creator"} or (status == "restricted" and getattr(member, "is_member", True))
...
def create_advertising_mutual_tracking_router(session_factory: async_sessionmaker[AsyncSession]) -> Router:
    ...
    async def manual_private_entry(callback: CallbackQuery) -> None:
        async with session_factory() as session:
            rows = (await session.execute(select(Group.chat_id, Group.title).join(GroupOwner, GroupOwner.chat_id == Group.chat_id).where(GroupOwner.user_id == callback.from_user.id, GroupOwner.is_current.is_(True), Group.status == GroupStatus.active.value).order_by(Group.title, Group.chat_id))).all()
