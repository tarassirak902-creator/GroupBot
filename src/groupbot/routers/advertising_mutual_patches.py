...
def install_mutual_ui_patches() -> None:
    ...
    def deal_keyboard(deal: AdvertisingDeal, viewer_id: int) -> InlineKeyboardMarkup:
        ...
        if viewer_id == deal.seller_user_id and deal.status == "pending":
            rows.append([InlineKeyboardButton(text="✅ Принять", callback_data=f"ads:mutual:accept:{deal.id}"), InlineKeyboardButton(text="❌ Отклонить", callback_data=f"ads:mutual:reject:{deal.id}")])
        ...
        rows.append([InlineKeyboardButton(text="◀️ Мои продажи" if viewer_id == deal.seller_user_id else "📋 Мои покупки", callback_data="ads:my_sales" if viewer_id == deal.seller_user_id else "ads:my_buys")])
    ...
    if not getattr(advertising_module, "_manual_op_menu_installed", False):
        base_home_keyboard = advertising_module._advertising_keyboard
        def advertising_keyboard() -> InlineKeyboardMarkup:
            markup = base_home_keyboard()
            rows = list(markup.inline_keyboard)
            rows.insert(1, [InlineKeyboardButton(text="🔔 Подключить ОП", callback_data="ads:manual")])
            return InlineKeyboardMarkup(inline_keyboard=rows)
        advertising_module._advertising_keyboard = advertising_keyboard
        advertising_module._manual_op_menu_installed = True
