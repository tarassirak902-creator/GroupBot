...
async def _helper_profile_extra(session: AsyncSession, *, chat_id: int, user_id: int) -> tuple[str, int]:
    violation_count = int((await session.execute(
    ...
    assignment = (await session.execute(
        select(AdminAssignment).join(AdminRole, AdminRole.id == AdminAssignment.role_id).where(
    ...
    if mentor_id is None:
        audit_rows = (await session.execute(
            ...
            ).order_by(AuditLog.created_at.desc(), AuditLog.id.desc()).limit(20)
        )).scalars().all()
    ...
    mentor = (await session.execute(select(User).where(User.telegram_user_id == mentor_id))).scalar_one_or_none()
    mentor_text = clickable_user_display(mentor) if mentor is not None else clickable_identity(
        telegram_user_id=mentor_id, first_name="Наставник", username=None,
    )
    return mentor_text, violation_count


async def _admin_profile_extra(session: AsyncSession, *, chat_id: int, user_id: int) -> tuple[int, int, int, int]:
    action_rows = (await session.execute(
        select(ModerationAction.action, func.count()).where(
            ModerationAction.chat_id == chat_id,
            ModerationAction.actor_user_id == user_id,
            ModerationAction.action.in_(["warning", "mute", "ban"]),
            ModerationAction.source != "warning_scale",
        ).group_by(ModerationAction.action)
    )).all()
    action_counts = {str(action): int(count) for action, count in action_rows}
    helper_count = int((await session.execute(
        select(func.count()).select_from(AdminAssignment).join(AdminRole, AdminRole.id == AdminAssignment.role_id).where(
            AdminAssignment.chat_id == chat_id,
    ...
    return action_counts.get("warning", 0), action_counts.get("mute", 0), action_counts.get("ban", 0), helper_count


async def _admin_leaderboard_text(session: AsyncSession, *, chat_id: int) -> str:
    owner_ids = set((await session.execute(select(GroupOwner.user_id).where(
        GroupOwner.chat_id == chat_id, GroupOwner.is_current.is_(True),
    ))).scalars().all())
    admin_rows = (await session.execute(
        select(AdminAssignment.user_id, AdminRole.name).join(AdminRole, AdminRole.id == AdminAssignment.role_id).where(
            AdminAssignment.chat_id == chat_id,
            AdminRole.is_active.is_(True),
            AdminRole.name != HELPER_ROLE,
        )
    ...
    action_rows = (await session.execute(
        select(ModerationAction.actor_user_id, ModerationAction.action, func.count()).where(
            ModerationAction.chat_id == chat_id,
    ...
    helper_rows = (await session.execute(
        select(AdminAssignment.user_id, AdminAssignment.assigned_by_user_id).join(AdminRole, AdminRole.id == AdminAssignment.role_id).where(
            AdminAssignment.chat_id == chat_id,
    ...
    report_rows = (await session.execute(select(AuditLog.payload).where(
        AuditLog.chat_id == chat_id,
        AuditLog.event_type == "group.helper_violation_reported",
    ))).scalars().all()
    for payload in report_rows:
        try:
            mentor_id = int((payload or {}).get("assigned_admin_id"))
        except (TypeError, ValueError):
            continue
        if mentor_id in admin_ids:
            helper_reports_by_mentor[mentor_id] = helper_reports_by_mentor.get(mentor_id, 0) + 1
    users = (await session.execute(select(User).where(User.telegram_user_id.in_(admin_ids)))).scalars().all()
    ...
    for user_id in admin_ids:
        ...
        warnings, mutes, bans = counts.get("warning", 0), counts.get("mute", 0), counts.get("ban", 0)
        helper_reports = helper_reports_by_mentor.get(user_id, 0)
        ranking.append((user_id, warnings + mutes + bans, helper_reports, warnings, mutes, bans))
    ranking.sort(key=lambda row: (-row[1], -row[2], row[0]))
    medals = ["🥇", "🥈", "🥉"]
    lines = ["🏆 <b>Топ администрации</b>", "", "Сортировка: по количеству фактических предов, мутов и банов.", ""]
    for index, (user_id, total, helper_reports, warnings, mutes, bans) in enumerate(ranking[:10], start=1):
        user = users_by_id.get(user_id)
        identity = clickable_user_display(user) if user is not None else clickable_identity(
            telegram_user_id=user_id, first_name="Администратор", username=None,
        )
...
async def _profile_text(session: AsyncSession, *, chat_id: int, target: TelegramUser) -> str:
    user = (await session.execute(select(User).where(User.telegram_user_id == target.id))).scalar_one_or_none()
    member = (await session.execute(select(GroupMember).where(
        GroupMember.chat_id == chat_id, GroupMember.user_id == target.id,
    ))).scalar_one_or_none()
    ...
    helper_mentor_text, helper_violation_count = "⚠️ не определён", 0
    if rank == HELPER_ROLE:
        helper_mentor_text, helper_violation_count = await _helper_profile_extra(session, chat_id=chat_id, user_id=target.id)
    admin_warnings = admin_mutes = admin_bans = admin_helpers = 0
    if owner or (rank is not None and rank != HELPER_ROLE):
        admin_warnings, admin_mutes, admin_bans, admin_helpers = await _admin_profile_extra(
            session, chat_id=chat_id, user_id=target.id,
        )
    ...
    admin_stats_line = (
        ...
        if owner or (rank is not None and rank != HELPER_ROLE) else ""
    )
    return (
        "👤 <b>Профиль участника</b>\n\n"
        f"Пользователь: {identity}\nСтатус в группе: <b>{status_text}</b>\n"
        f"Ранг Mimorus: <b>{admin_line}</b>\nОсобый статус: <b>{special_line}</b>{helper_line}\n\n"
        ...
        f"Сообщений учтено: <b>{messages}</b>\nАктивных предупреждений: <b>{warnings}</b>{admin_stats_line}"
    )
...
def create_group_text_aliases_router(session_factory: async_sessionmaker[AsyncSession]) -> Router:
    ...
    @router.message(F.chat.type.in_({"group", "supergroup"}), F.text.regexp(r"(?i)^\s*кто\s+я\s*[?？]?\s*$"))
    async def who_am_i(message: Message) -> None:
        ...
        async with session_factory() as session:
            if not await _access_allowed(session, message.chat.id):
                return
            text = await _profile_text(session, chat_id=message.chat.id, target=message.from_user)
    ...
    async def who_is_reply_target(message: Message) -> None:
        ...
        async with session_factory() as session:
            if not await _access_allowed(session, message.chat.id):
                return
            text = await _profile_text(session, chat_id=message.chat.id, target=target)
        await message.answer(text, parse_mode="HTML", disable_web_page_preview=True)
