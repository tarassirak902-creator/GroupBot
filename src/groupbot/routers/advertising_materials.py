...
def _materials_keyboard(deal: AdvertisingDeal, placements: list[AdvertisingPlacement]) -> InlineKeyboardMarkup:
    ...
    if deal.requested_post:
        ...
        if post is None or post.status in {"pending", "ready"}:
    ...
    if deal.requested_mandatory:
        mandatory = by_kind.get("mandatory")
        if mandatory is None or mandatory.status in {"pending", "ready"}:
            ready = mandatory is not None and mandatory.status == "ready"
            rows.append([InlineKeyboardButton(
...
def _materials_text(deal: AdvertisingDeal, listing: AdvertisingListing, placements: list[AdvertisingPlacement]) -> str:
    by_kind = {row.kind: row for row in placements}
    lines = ["📦 <b>Материалы рекламной сделки</b>", "", f"🏠 Площадка: <b>{listing.group_title_snapshot}</b>", ""]
    if deal.requested_post:
        post = by_kind.get("post")
        if post is not None and post.status == "active":
    ...
    if deal.requested_mandatory:
        ...
        if placement is not None and placement.status == "active":
...
def create_advertising_materials_router(session_factory: async_sessionmaker[AsyncSession]) -> Router:
    ...
    async def _load(deal_id: int) -> tuple[AdvertisingDeal, AdvertisingListing, list[AdvertisingPlacement]] | None:
        async with session_factory() as session:
            ...
            if listing is None:
                return None
            placements = list((await session.execute(select(AdvertisingPlacement).where(AdvertisingPlacement.deal_id == deal_id).order_by(AdvertisingPlacement.id))).scalars().all())
    ...
    async def _notify_if_ready(bot: Bot, deal_id: int) -> bool:
        ...
        try:
            await bot.send_message(
                ...
                "Покупатель передал все необходимые материалы.",
                parse_mode="HTML",
    ...
    async def materials_home(callback: CallbackQuery, state: FSMContext) -> None:
        ...
        if callback.from_user.id != deal.buyer_user_id:
            ...
            return
        if deal.status not in {"accepted", "finished_waiting_confirmation"}:
            await callback.answer("Материалы сейчас недоступны.", show_alert=True)
            return
        await state.clear()
        await callback.message.edit_text(_materials_text(deal, listing, placements), parse_mode="HTML", reply_markup=_materials_keyboard(deal, placements))
    ...
    async def choose_post(callback: CallbackQuery, state: FSMContext) -> None:
        ...
        await callback.message.edit_text(
            "📣 <b>Рекламный пост</b>\n\nОтправьте или перешлите сюда готовый рекламный пост.",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="❌ Отмена", callback_data=f"ads:materials:{deal_id}")]]),
        )
        await callback.answer()
    ...
    async def receive_post(message: Message, state: FSMContext, bot: Bot) -> None:
        ...
        await state.clear()
        await message.answer("✅ Рекламный пост сохранён.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="📦 Материалы сделки", callback_data=f"ads:materials:{deal_id}")]]))
    ...
    async def receive_mandatory(message: Message, state: FSMContext, bot: Bot) -> None:
        ...
        await message.answer(
            ...
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="📦 Материалы сделки", callback_data=f"ads:materials:{deal_id}")]]),
        )
        await _notify_if_ready(bot, deal_id)
