...
async def _available_listing(session: AsyncSession, *, listing_id: int, buyer_user_id: int) -> AdvertisingListing | None:
...
async def _mandatory_target_ready(bot: Bot, session: AsyncSession, *, target_chat_id: int, buyer_user_id: int) -> bool:
    ...
    if group is not None:
        return await _owned_active_group(session, chat_id=target_chat_id, owner_user_id=buyer_user_id)
    try:
...
def _unavailable_text() -> str:
    return "Эта рекламная площадка сейчас недоступна: объявление выключено, группа отключена, владелец сменился или подписка закончилась."


def create_advertising_request_guard_router(session_factory: async_sessionmaker[AsyncSession]) -> Router:
    ...
    async def accept_mandatory(callback: CallbackQuery, bot: Bot) -> None:
        ...
        async with session_factory() as session:
            async with session.begin():
                ...
                if listing is None or not await _seller_listing_ready(session, listing=listing, seller_user_id=callback.from_user.id):
                    await callback.answer("Заявку нельзя запустить: рекламная группа отключена, сменила владельца или подписка закончилась.", show_alert=True); return
                used_op = int((await session.execute(select(func.count()).select_from(AdvertisingDeal).where(AdvertisingDeal.listing_id == deal.listing_id, AdvertisingDeal.accepted_at >= day_start, AdvertisingDeal.requested_mandatory.is_(True)))).scalar_one())
                ...
                if mandatory is None or mandatory.status != "ready":
                    await callback.answer("Данные ОП не готовы.", show_alert=True); return
                cfg = dict(mandatory.config_json or {})
                target_chat_id = cfg.get("target_chat_id")
                if not isinstance(target_chat_id, int):
                    await callback.answer("Группа ОП не определена.", show_alert=True); return
                if not await _mandatory_target_ready(bot, session, target_chat_id=target_chat_id, buyer_user_id=deal.buyer_user_id):
                    await callback.answer("ОП нельзя запустить: целевая группа отключена, сменила владельца, потеряла подписку или Mimorus больше не имеет к ней доступа.", show_alert=True); return
                ...
                if mode == "days":
                    ...
                    try:
                        ...
                        await callback.answer("Не удалось получить текущее число участников целевой группы. ОП не запущена.", show_alert=True); return
                ...
                if deal.requested_post:
                    ...
                    if post is None or post.status != "ready":
                        await callback.answer("Рекламный пост не готов.", show_alert=True); return
        ...
        if buyer_id is not None:
            try:
                ...
                await bot.send_message(buyer_id, "✅ <b>Рекламодатель одобрил вашу заявку</b>\n\n" f"🏠 Площадка: <b>{escape(seller_group)}</b>\n" f"🎯 ОП на: <a href=\"{target_url}\">{escape(target_title)}</a>\n" f"📐 Объём ОП: <b>{volume_text}</b>\n" f"⭐ Стоимость ОП: <b>{total_price} ⭐</b>\n\n" f"🚀 Обязательная подписка включена автоматически.{extra}", parse_mode="HTML", disable_web_page_preview=True)
            except Exception:
                pass
        if callback.message is not None:
            extra = f"\n📣 Пост: <b>запущен на {duration_days} дн.</b>" if post_started else ""
            await callback.message.edit_text("✅ <b>Заявка одобрена и запущена</b>\n\n" f"🎯 ОП: <a href=\"{target_url}\">{escape(target_title)}</a>\n" f"📐 Объём: <b>{volume_text}</b>{extra}\n\n" "Обычные участники вашей группы должны быть подписаны на указанную площадку, чтобы писать сообщения.", parse_mode="HTML", disable_web_page_preview=True)
