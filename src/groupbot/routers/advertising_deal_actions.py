...
def create_advertising_deal_actions_router(
    ...
    async def accept_deal(callback: CallbackQuery, bot: Bot) -> None:
        ...
        async with session_factory() as session:
            async with session.begin():
                ...
                if deal.requested_post:
                    ...
                    if used_post >= POST_DAILY_LIMIT:
                        await callback.answer(
                            "Лимит рекламных постов на сегодня уже использован: 1 из 1.",
                            show_alert=True,
                        )
                        return

                if deal.requested_mandatory:
                    used_mandatory = int((await session.execute(
                        select(func.count())
                        .select_from(AdvertisingDeal)
                        .where(
                            AdvertisingDeal.listing_id == deal.listing_id,
                    ...
                    if used_mandatory >= MANDATORY_DAILY_LIMIT:
                        await callback.answer(
                            "Лимит ОП на сегодня уже использован: 3 из 3.",
                            show_alert=True,
                        )
                        return
                ...
                if deal.requested_mandatory:
                    existing_mandatory = (await session.execute(
                        select(AdvertisingPlacement).where(
                            AdvertisingPlacement.deal_id == deal.id,
                            AdvertisingPlacement.kind == "mandatory",
                        )
                    )).scalar_one_or_none()
                    if existing_mandatory is None:
                        session.add(AdvertisingPlacement(
                            deal_id=deal.id,
                            kind="mandatory",
                            status="pending",
        ...
        if callback.message is not None:
            await callback.message.edit_text(
                ...
                f"🏠 Площадка: <b>{title}</b>\n"
                f"📌 Формат: <b>{kind_label}</b>\n"
                "Статус: <b>ожидает материалов покупателя</b>\n\n"
